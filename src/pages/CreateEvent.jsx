...
export default function CreateEvent() {
  ...
  const handleDateChange = (val) => {
    setForm(f => {
      ...
      return { ...f, date: val, end_time: endStr };
    });
  };
  ...
  const handleEndTimeChange = (val) => {
    if (form.date && val) {
      ...
      if (diffH > 24) { alert(lang === 'cs' ? 'Maximální délka akce je 24 hodin' : 'Maximum event duration is 24 hours'); return; }
      if (diffH < 0) { alert(lang === 'cs' ? 'Konec akce musí být po začátku' : 'End time must be after start time'); return; }
    }
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <DateTimePicker
                ...
                placeholder={lang === 'cs' ? 'Datum a čas' : 'Date & time'}
            ...
            <div>
              ...
              <DateTimePicker
                ...
                placeholder={lang === 'cs' ? 'Konec' : 'End time'}
                minDate={form.date || new Date().toISOString()}
              />
            </div>
          </div>
