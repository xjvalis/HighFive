...
function scoreEvent(event, profile, userEmail, isPersonalized) {
  ...
  if (profile?.age) {
    ...
    const inRange =
      (!event.age_min || age >= event.age_min) &&
      (!event.age_max || age <= event.age_max);
    if (inRange) score += 10;
    else score -= 20;
  }

  // Upcoming soon (next 7 days) — boost relevance
  const daysUntil = (eventDate - now) / (1000 * 60 * 60 * 24);
  if (daysUntil >= 0 && daysUntil <= 7) score += 15;
  else if (daysUntil > 7 && daysUntil <= 30) score += 5;

  // Recency of creation — slight boost for newer
...
export default function FeedList({ events, user, profile, onJoin, onFavorite, isPersonalized = false }) {
  const tr = useT();
  const { featured, personalizedRest, regularRest } = useMemo(() => {
    ...
    if (isPersonalized) {
      const scored = nonFeatured.map(e => ({
        event: e,
        score: scoreEvent(e, profile, user?.email, true),
      }));
      scored.sort((a, b) => b.score - a.score);
      return {
        featured: shuffledFeatured,
        personalizedRest: scored.map(s => s.event),
        regularRest: [],
      };
    }
  ...
  // In personalized mode, group: your categories first, then rest
  ...
  // Insert a subtle divider after featured if there are more
  return (
    <div className="space-y-2 sm:space-y-2.5">
      {allSorted.map((event, i) => (
        <div key={event.id}>
          {/* Subtle separator after featured block */}
          ...
          {/* Motivation message every 4-5 cards */}
          {i > 0 && (i + 1) % 4 === 0 && <FeedMotivation events={events} index={Math.floor(i / 4)} />}
          {renderCard(event)}
        </div>
      ))}
    </div>
