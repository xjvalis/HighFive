  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        ...
        {tr.filterButton}
        {activeCount > 0 && (
          <span className="bg-primary text-primary-foreground rounded-full w-4 h-4 flex items-center justify-center text-[10px] leading-none">
            {activeCount}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-full mt-2 bg-card border border-border rounded-2xl shadow-lg z-30 p-4 w-64 space-y-3">
          ...
          <div>
            ...
            <div className="flex gap-1.5 mb-1.5">
              <button
                ...
                className={cn(
                  "flex-1 py-1.5 rounded-xl text-[11px] font-medium border transition-all",
              ...
              <button
                ...
                className={cn(
                  "flex-1 py-1.5 rounded-xl text-[11px] font-medium border transition-all",
                  filters.date === (() => { const d = new Date(); d.setDate(d.getDate()+1); return d.toISOString().split("T")[0]; })()
                    ? "bg-lavender border-violet-200 text-violet-700"
                    : "bg-secondary border-transparent text-muted-foreground hover:text-foreground"
                )}
              >
          ...
          <div>
            ...
            <Input
              placeholder={tr.filterLocationPlaceholder}
          ...
          <div>
            ...
            <Input
              type="number"
              min={1}
              placeholder={tr.filterMaxPeoplePlaceholder}
          ...
          <div>
            ...
            <div className="flex gap-1">
              {[{ val: "", label: tr.filterAll }, { val: "free", label: tr.filterFree }, { val: "paid", label: tr.filterPaid }].map(opt => (
                <button
                  key={opt.val}
                  onClick={() => update("paid", opt.val)}
                  className={cn(
                    "flex-1 py-1.5 rounded-xl text-[11px] font-medium border transition-all",
                    filters.paid === opt.val
                      ? "bg-lavender border-violet-200 text-violet-700"
                      : "bg-secondary border-transparent text-muted-foreground hover:text-foreground"
                  )}
                >
